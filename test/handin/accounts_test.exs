defmodule Handin.AccountsTest do
  ...
  describe "get_user_by_email/1" do
    ...
    test "returns the user if the email exists" do
      %{id: id} = user = insert!(:user_unconfirmed)
  ...
  describe "get_user_by_email_and_password/2" do
    ...
    test "does not return the user if the password is not valid" do
      user = insert!(:user_unconfirmed)
      refute Accounts.get_user_by_email_and_password(user.email, "invalid")
    end

    test "returns the user if the email and password are valid" do
      %{id: id} = user = insert!(:user_unconfirmed) |> verify_user()
  ...
  describe "register_user/1" do
    setup do
      ...
      %{user: insert!(:user_unconfirmed, university: university.id), university: university}
    end

    test "requires email, password and university to be set" do
      {:error, changeset} = Accounts.register_user(%{})

      assert %{
               password: ["can't be blank"],
               email: ["select a university"],
  ...
  describe "change_user_registration/2" do
    ...
    test "returns a changeset" do
      ...
      assert changeset.required == [:password, :university]
    end

    test "allows fields to be set", %{university: university} do
      email = unique_user_email()
      password = valid_user_password()
      ...
      changeset =
        Accounts.change_user_registration(
          %User{},
          %{email: email, password: password, university: university.id}
        )
  ...
  describe "apply_user_email/3" do
    setup do
      university = insert!(:university)
      %{user: insert!(:user_unconfirmed, university: university.id)}
    end
    ...
    test "validates email", %{user: user} do
      ...
      assert %{email: ["please use your university student email address"]} = errors_on(changeset)
    end
    ...
    test "generates a token", %{user: user} do
      ...
      assert_raise Ecto.ConstraintError, fn ->
        Repo.insert!(%UserToken{
          ...
          user_id: insert!(:user_unconfirmed).id,
          context: "session"
        })
      end
    end
