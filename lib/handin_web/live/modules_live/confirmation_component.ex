defmodule HandinWeb.ModulesLive.ConfirmationComponent do
  use HandinWeb, :live_component

  alias Handin.Modules
  alias Handin.Modules.{Module, CloneModuleParams}
  ...
  def handle_event("clone", %{"id" => id}, socket) do
    clone_module_params = %CloneModuleParams{
      module_id: id,
      user_id: socket.assigns.user_id,
      timezone: socket.assigns.timezone
    }

    case Modules.clone_module(clone_module_params) do
      {:ok, %Module{} = cloned_module} ->
        notify_parent({:cloned, cloned_module})
  ...
  def handle_event("archive", %{"id" => id}, socket) do
    with %Module{} = module <- Modules.get_module!(id),
         {:ok, module} <- Modules.archive_module(module) do
      notify_parent({:archived, module})
